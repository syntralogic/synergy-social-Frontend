...
import { MapPin, Link2, Calendar, BadgeCheck, Grid3X3, Heart, MessageCircle, Settings, Camera, Loader2, CheckCircle, X } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    // Try to get posts by this user from feed, filter by author
    postsAPI.getFeed(1)
      .then((res: any) => {
        const all = res.data || [];
  ...
  // Upload image as base64 (or multipart if backend supports)
  const uploadImage = async (file: File, type: 'avatar' | 'cover') => {
    const setter = type === 'avatar' ? setAvatarUploading : setCoverUploading;
    setter(true);
    try {
      // Convert to base64 dataURL and send to backend
      ...
      // Try updating profile with image URL (as base64 or use a temp object URL)
      // Send as JSON field - backend may or may not support this
      ...
      if (res?.data?.user) {
        ...
        // Locally update even if backend doesn't save it
        setUser({ ...currentUser!, [field]: dataUrl });
      }
    } catch {
      // Locally preview even if upload fails
  ...
  const coverBg = (currentUser as any)?.coverImage
    ? `url(${(currentUser as any).coverImage}) center/cover`
      ...
      <div style={{ padding:'0 24px', position:'relative', marginTop:-48 }}>
        ...
        <div style={{ display:'flex', marginBottom:20, borderBottom:'1px solid var(--border)' }}>
          {(['posts','liked','saved'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              style={{ padding:'10px 20px', background:'none', border:'none', fontSize:14, fontWeight: tab===t ? 700 : 400,
        ...
        {loadingPosts ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6 }}>
            {[...Array(9)].map((_,i) => (
              <div key={i} className="shimmer-bg" style={{ aspectRatio:'1', borderRadius:8, background:'var(--bg3)' }}/>
            ))}
          </div>
        ) : myPosts.length === 0 ? (
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6, paddingBottom:32 }}>
            {myPosts.map((p, i) => (
              <motion.div key={p.id} whileHover={{ scale:1.02, zIndex:2 }}
                style={{ aspectRatio:'1', borderRadius:8, overflow:'hidden', background:'var(--bg3)', cursor:'pointer', position:'relative' }}>
                {p.media?.[0] ? (
                  <>
                    <img src={p.media[0].mediaUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }}/>
                    <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,0)', transition:'background 0.2s', display:'flex', alignItems:'center', justifyContent:'center', gap:12 }}
                      onMouseEnter={e => { const d=e.currentTarget as HTMLDivElement; d.style.background='rgba(0,0,0,.5)'; d.querySelectorAll('span').forEach(s => (s as HTMLElement).style.opacity='1'); }}
                      onMouseLeave={e => { const d=e.currentTarget as HTMLDivElement; d.style.background='rgba(0,0,0,0)'; d.querySelectorAll('span').forEach(s => (s as HTMLElement).style.opacity='0'); }}>
                      <span style={{ color:'white', fontSize:13, fontWeight:700, display:'flex', alignItems:'center', gap:4, opacity:0, transition:'opacity 0.2s' }}><Heart size={14} fill="white"/> {fmtNum(p._count.likes)}</span>
                      <span style={{ color:'white', fontSize:13, fontWeight:700, display:'flex', alignItems:'center', gap:4, opacity:0, transition:'opacity 0.2s' }}><MessageCircle size={14}/> {fmtNum(p._count.comments)}</span>
                    </div>
                  </>
                ) : (
                  <div style={{ width:'100%', height:'100%', display:'flex', flexDirection:'column', padding:12 }}>
                    <p style={{ fontSize:12, color:'var(--text2)', lineHeight:1.5, flex:1, overflow:'hidden' }}>{p.content?.slice(0,80)}{p.content?.length > 80 ? '…' : ''}</p>
                    <div style={{ fontSize:11, color:'var(--text3)', marginTop:6 }}>❤ {fmtNum(p._count.likes)}</div>
                  </div>
                )}
              </motion.div>
            ))}
